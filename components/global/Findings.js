...
function Findings({ children, className }) {
  ...
  // const hovered = useHoverAndFocus(reference, {elems:'current'}, 'hover'); //current receives a hover class when hovered, and it is removed when it is blurred
  // // const hovered = useHoverAndFocus(reference, {elems:'current', on:'on--class', off:'off--class'}); //current receives an on--class when hovered, and an off--class when blurred, and both are removed when the other is added
  // // const hovered = useHoverAndFocus(reference, [{elems:'test--class', on:'test--class__on', off:'test--class__off'}, {elems:'current', on:'on', off:'off'}]); //even when the second variable is an array, the hook is still able to recognize `current` and treat it differently then other `elems` values.  Current receives an on--class when hovered, and an off--class when blurred, and both are removed when the other is added, on top of the other changes that are made to the other elements in the array.
